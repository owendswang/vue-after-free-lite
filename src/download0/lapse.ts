...
export function lapse () {
  ...
  function race_one (req_addr: BigInt, tcp_sd: BigInt, sds: BigInt[]): [BigInt, BigInt] | null {
    try {
      ...
      error('  race_one error: ' + (e as Error).message)
      return null
    }
  }
  ...
  function double_free_reqs2 (): [BigInt, BigInt] | null {
    try {
      ...
      error('Stage 1 error: ' + (e as Error).message)
      return null
    }
  }
  ...
  function make_kernel_arw (pktopts_sds: BigInt[], reqs1_addr: BigInt, kernel_addr: BigInt, sds: BigInt[], sds_alt: BigInt[], aio_info_addr: BigInt) {
    try {
      ...
      error('make_kernel_arw error: ' + (e as Error).message)
      return null
    }
  }
  ...
  try {
    ...
    if (!setup_success) {
      error('Setup failed')
      utils.notify('Lapse: Setup failed')
      return false
    }
    log('Setup completed')
    ...
    if (sd_pair === null) {
      log('[FAILED] Stage 1')
      utils.notify('Lapse: FAILED at Stage 1')
      return false
    }
    ...
    if (sds === null) {
      ...
      utils.notify('Lapse: FAILED at Stage 1 (sds creation)')
      return false
    }
    ...
    if (kpatch_result) {
      ...
      try {
        ...
        error('  [FAIL] mmap RWX test error: ' + (e as Error).message)
        all_patches_ok = false
      }
    ...
    error('Lapse error: ' + (e as Error).message)
